...
const JwtRefreshStrategy = passportJWT.Strategy;
const jwtRefreshSecret = process.env.JWT_REFRESH_SECRET || 'asdsad';
passport.use('jwt-refresh', new JwtRefreshStrategy({
  jwtFromRequest: ExtractJwt.fromAuthHeaderAsBearerToken(),
  ...
  //   iat: 1706485225,
  //  exp: 1709077225,
  //  refreshToken: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2NWI2YmNkNzFjNzJkNzczZjVkYmFhNjUiLCJwaG9uZU51bWJlciI6IjQzNy01NTYtNDAzNSIsIml
  // // hdCI6MTcwNjQ4NTIyNSwiZXhwIjoxNzA5MDc3MjI1fQ.dVTqgiaDgjBrgTw_6MgSa0zrqPl3vrsiwSLgR86h7vU'
  // // auth-service          | }
  const refreshToken = req.get('Authorization')?.replace('Bearer', '').trim();
  done(null, { ...jwtPayload, refreshToken });
}));
